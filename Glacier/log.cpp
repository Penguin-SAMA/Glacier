...
void Logger::setFormatter(const std::string& val) {
    ...
    Glacier::LogFormatter::ptr new_val(new Glacier::LogFormatter(val));
    if (new_val->isError()) {
...
void FileLogAppender::log(Logger::ptr logger, LogLevel::Level level, LogEvent::ptr event) {
    if (level >= m_level) {
        ...
        if (!m_formatter->format(m_filestream, logger, level, event)) {
            std::cout << "FileLogAppender log failed" << std::endl;
        }
    }
}
...
void StdoutLogAppender::log(Logger::ptr logger, LogLevel::Level level, LogEvent::ptr event) {
    if (level >= m_level) {
        MutexType::Lock lock(m_mutex);
        if (!m_formatter->format(std::cout, logger, level, event)) {
            std::cout << "StdoutLogAppender log failed" << std::endl;
        }
    }
}
...
Glacier::ConfigVar<std::set<LogDefine>>::ptr g_log_defines = Glacier::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

struct LogIniter {
    LogIniter() {
        g_log_defines->addListener([](const std::set<LogDefine>& old_value, const std::set<LogDefine>& new_value) {
            GLACIER_LOG_INFO(GLACIER_LOG_ROOT()) << "on_logger_conf_changed";
            for (auto& i : new_value) {
                auto it = old_value.find(i);
                Glacier::Logger::ptr logger;
                if (it == old_value.end()) {
                    // 新增logger
                    logger = GLACIER_LOG_NAME(i.name);
                } else {
                    if (!(i == *it)) {
                        // 修改的logger
                        logger = GLACIER_LOG_NAME(i.name);
            ...
            for (auto& i : old_value) {
                ...
                if (it == new_value.end()) {
                    // 删除logger
                    auto logger = GLACIER_LOG_NAME(i.name);
